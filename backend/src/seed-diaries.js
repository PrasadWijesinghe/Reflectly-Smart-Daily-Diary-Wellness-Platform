...
const DIARY_DATA = [
  ...
  {
    daysAgo: 8,
    summary: "Group project struggles",
...
async function seed() {
  ...
  const user = await prisma.user.upsert({
    ...
    create: {
      name: "Lakshan",
      email: email,
      password: "password123", // Dummy password in case the user doesn't exist
    },
  });
  ...
  // Calculate the starting date directly from midnight
  const now = new Date();
  ...
  for (const data of DIARY_DATA) {
    ...
    // Normalize date to prevent duplicate days issues
    ...
    console.log(`Seeded diary for ${targetDate.toDateString()}: ${data.summary}`);
  }
